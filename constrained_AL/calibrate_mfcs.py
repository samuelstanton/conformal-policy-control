...
def get_pool_norms(Xpool, gpr_model, lmbda):
    '''
    Xpool           : dim (n + t, d), array where each row is a data input *in pool set* of dimension d
    gpr_model       : Fitted sklearn GPRegression model for time T
    lmbda           : scalar, temperature/shift magnitude
    '''
    n_pool = np.shape(Xpool)[0]

    #### Compute for pool data
    ## Get posterior variances on pool data
    _, std_pool_ = gpr_model.predict(Xpool, return_std=True)
    std_pool = get_f_std(std_pool_, gpr_model)
    var_pool = std_pool**2

    ## Get min-max norm denominator for posterior variances
    var_pool_min_max_norm = max(var_pool) - min(var_pool)

    ## Get sum of exponentiated posterior variances on pool dataset
    var_pool_normed = var_pool / var_pool_min_max_norm
    exp_var_pool = np.exp(var_pool_normed * lmbda)
    exp_pool_sum_norm = np.sum(exp_var_pool)

    return var_pool_min_max_norm, exp_pool_sum_norm
    

    

def simple_pdf_gpr(X, var_pool_min_max_norm, exp_pool_sum_norm, gpr_model, lmbda):
    '''
    ...
    #### For cal&test data
    ## Get posterior variances on cal&test data
    ...
    pdf_vals = np.exp(var_pred_normed * lmbda) / exp_pool_sum_norm
...
def compute_risk_control_weights(X_cal, Feasible_cal, var_pool_min_max_norm, exp_var_preds_pool_split, source_densities_cal,\
                                 source_densities_pool, gpr_model, lmbda, source_pdf, alpha=0.5):
    '''
    X_cal                     : dim (n + t - 1, d), array where each row is a data input of dimension d
    Feasible_cal              : dim (n + t - 1), array of Feasible set indicators for cal data (not test data)
    var_pool_min_max_norm     : scalar, max(var_pool) - min(var_pool)
    ...
    N = np.shape(X_cal)[0]
    exp_var_preds_pool_split_mean = np.max(exp_var_preds_pool_split) ## Changed to max
    exp_var_preds_pool_split_sum = np.sum(exp_var_preds_pool_split)

    exp_var_preds_pool_split_normed = exp_var_preds_pool_split / exp_var_preds_pool_split_sum

    ## Check risk level in source distribution:
    # source_densities_cal = source_pdf(X_cal)
    # source_densities_pool = source_pdf(Xpool_split)
    source_densities_pool_mean = np.max(source_densities_pool)
    source_weights_cal_pool_mean = np.concatenate((source_densities_cal, [source_densities_pool_mean]))
    source_weights_cal_pool_mean_normalized = source_weights_cal_pool_mean / np.sum(source_weights_cal_pool_mean)
    if (np.sum(source_densities_cal[Feasible_cal == 0]) + source_densities_pool_mean > alpha):
        # raise Exception("Feasible set risk must be controlled in source distribution to begin")
        print("Warning: Feasible set risk not controlled in source distribution to begin")
        return source_weights_cal_pool_mean_normalized, 1
    ...
    ## TO DO: This should be cal + test (averaged over pool) data

    ## Find the smallest risk-controlling parameter
    pc_params_grid = np.arange(start=0, stop=1.01, step=0.025)
    
    for pc_param in pc_params_grid:
        ## Compute average proposal density 
        # pool_mixed_proposal_densities = pc_param * source_densities_pool + (1 - pc_param) * exp_var_preds_pool_split_normed
        pool_mixed_proposal_densities_mean = np.max(exp_var_preds_pool_split_normed)

        pdf_vals_cal_test = np.concatenate((pdf_vals, [pool_mixed_proposal_densities_mean]))
        
        
        mixture_weights = pc_param * source_weights_cal_pool_mean + (1 - pc_param) * pdf_vals_cal_test
        mixture_weights_normalized = mixture_weights / np.sum(mixture_weights)
        # print(f'sum weights for pc_param={} : {np.sum(mixture_weights[:-1][Feasible_cal == 0]) + mixture_weights[-1]}')
        if (np.sum(mixture_weights_normalized[:-1][Feasible_cal == 0]) + mixture_weights_normalized[-1] <= alpha):
            break
    
    return mixture_weights, pc_param
    
    

##### Added 20250627 #####
def mixture_pdf_gpr(X, var_pool_min_max_norms, exp_pool_sum_norms, mixture_weights, gpr_models, lmbda):
    '''
    X                       : dim (n + t, d), array where each row is a data input of dimension d
    var_pool_min_max_norms  : dim (T), list where var_pool_min_max_norms[t] = max(var_pool) - min(var_pool) for all t > 0
    exp_pool_sum_norms      : dim (T), list where exp_pool_sum_norms[t] = np.exp(var_pool_normed * lmbda) for all t > 0
    ...
    gpr_models              : dim (T), array where each entry is a fitted sklearn GPRegression model, from T=0, ..., T-1
                                 Note: gpr_models[0] = source_pdf OR None
    lmbda                   : scalar, temperature/shift magnitude
    '''
    if (X.ndim != 2):
        raise Exception("X must be array of dimension 2; if X.ndim == 1 and each entry is a different sample, do: X = X.reshape(-1,1); if is one datapoint, do X = X.reshape(1,-1)")

    if (len(mixture_weights) != len(gpr_models)):
        raise Exception("len(mixture_weights) != len(gpr_models)")
        
    N = np.shape(X)[0]
    T = len(mixture_weights)
    mixture_weights_normed = mixture_weights / np.sum(mixture_weights)

    ## Now compute densities for each point 
    pdfs_NxT = np.zeros((N, T)) ## dim (N, T); pdfs_NxT[i, t] is density of i-th point for t-th proposal distribution
    # pdfs_NxT[:, 0] = np.ones(N) / N ## For t = 0: Uniform weight on all points ## computing this for selection bias
    
    for t, w in enumerate(mixture_weights_normed):
        ## t == 0 case: gpr_models[0] is 'source_pdf' function
        if (t == 0):
            pdfs_NxT[:,t] = gpr_models[t](X)
            
        elif (t > 0):
            ## Get posterior variances on pool data
            _, std_pred_ = gpr_models[t].predict(X, return_std=True)
            std_pred = get_f_std(std_pred_, gpr_models[t])
            var_pred = std_pred**2
    
            ## Get sum of exponentiated posterior variances on pool dataset
            var_pred_normed = var_pred / var_pool_min_max_norms[t]
            pdfs_NxT[:,t] = np.exp(var_pred_normed * lmbda) / exp_pool_sum_norms[t]

    mixture_pdfs_T = pdfs_NxT @ mixture_weights_normed

    return mixture_pdfs_T
...
#                 ## Note: For replacement case, easier to normalize ahead of time by dividing by Z
    ...
    def compute_confidence_sets_active(self, Xtrain_split, Xcal_split, ytrain_split, ycal_split, Xtest_n1xp_split, ytest_n1_split, Xpool_split, w_split_mus_prev_steps, exp_vals_pool_list_of_vecs_all_steps, constrained_densities_cal_test_running_list, constrained_densities_pool_running_list, pc_weights_cal_test_normalized, var_pool_min_max_norms, exp_pool_sum_norms, method_names, pc_densities_cal_test, pc_densities_pool, t_cal, X_dataset, n_cal_initial, alpha_aci_curr, weight_bounds, source_pdf=None, pc_params_list=[None], weight_depth_maxes = [1,2], lmbda = 1/10, bandwidth = 1.0,
                                       alpha: float = 0.1, n_initial_all=100, n_dataset = None, replacement=True, record_weights=False):
        # , add_to_cal=True
        if (self.p != Xtrain_split.shape[1]):
            raise ValueError('Feature dimension {} differs from provided Xuniv_uxp {}'.format(
                Xtrain_split.shape[1], self.Xuniv_uxp.shape))
...
#         n1 = len(ytest_n1_split)
        ...
        # ## each calibration + test point at each step t \in {1,...t_cal}
        # ## Also, compute array of length t_cal for the sum of query function values for all pool points at that step
        # constrained_density_ratios_cal_test_all_steps = np.zeros((t_cal, n_cal + 1))
        # constrained_densities_cal_test_all_steps = np.zeros((t_cal, n_cal + 1))
        # # exp_vals_pool_sum_all_steps = np.zeros(t_cal)
        source_densities_cal_test = source_pdf(Xaug_cal_test_split)

        # for t in range(len(constrained_densities_cal_test_running_list)):
        #     print(f'constrained_densities_cal_test_running_list {t} : {len(constrained_densities_cal_test_running_list)}')

        # print(f'constrained_densities_cal_test_running_list : {np.shape(np.array(constrained_densities_cal_test_running_list))}')
        # print(f' np.array(constrained_densities_cal_test_running_list) shape : {np.shape(np.array(constrained_densities_cal_test_running_list))}')
        # for t in range(len(constrained_densities_cal_test_running_list)):
        #     print(f'len(constrained_densities_cal_test_running_list[t]) : {len(constrained_densities_cal_test_running_list[t])}')
        # print(f'np.tile(source_densities_cal_test, (t_cal, 1) shape : {np.shape(np.tile(source_densities_cal_test, (t_cal+1, 1)))}')

        # constrained_densities_cal_test_running_arr = np.array(constrained_densities_cal_test_running_list)
        ...
        #     source_densities_pool = source_pdf(Xpool_split)
        #     source_densities_cal_test = source_pdf(Xaug_cal_test_split)
        #     # proposal_densities_pool = simple_pdf_gpr(Xpool_split, var_pool_min_max_norms[i], exp_pool_sum_norms[i], muh_split_curr, lmbda)
        #     # constrained_densities_pool = pc_param * source_densities_pool + (1 - pc_param) * proposal_densities_pool

        #     ## Compute cal and test mixture densities
            
        #     # proposal_densities_cal_test = simple_pdf_gpr(Xaug_cal_test_split, var_pool_min_max_norms[i+1], exp_pool_sum_norms[i+1], muh_split_curr, lmbda)
        #     # constrained_densities_cal_test = pc_param * source_densities_cal_test + (1 - pc_param) * proposal_densities_cal_test
        #     constrained_densities_cal_test = constrained_pdf_gpr_lik_ratio(Xaug_cal_test_split, var_pool_min_max_norms[i+1], exp_pool_sum_norms[i+1], muh_split_curr, lmbda, pc_param, source_densities_pool, source_densities_cal_test, pc_densities_pool)

        #     ## Using likelihood ratios:
        #     constrained_density_ratios_cal_test_all_steps[i] = constrained_densities_cal_test / source_densities_cal_test
        #     constrained_densities_cal_test_all_steps[i] = constrained_densities_cal_test
    ...
    #                 ## Note: For replacement case, easier to normalize ahead of time by dividing by Z
        ...
        # mixture_weights = np.concatenate((np.array([n_cal_initial]), np.ones(len(w_split_mus_prev_steps_only))))

        
        # # Tmin1_mixture_pdf_vals = mixture_pdf_gpr(X=Xaug_cal_test_split, var_pool_min_max_norms=var_pool_min_max_norms[:-1], \
        # #                                          exp_pool_sum_norms=exp_pool_sum_norms[:-1], mixture_weights=mixture_weights, \
        # #                                          gpr_models=np.concatenate(([source_pdf], w_split_mus_prev_steps_only)), lmbda=lmbda)

        # ## Check fencepost problem here: Make sure 0:T-1 lines up between matrix and mixture_weights
        # # print(f'source_densities_cal_test shape            : {np.shape(source_densities_cal_test)}')
        # # print(f'constrained_densities_cal_test_all_steps shape : {np.shape(constrained_densities_cal_test_all_steps)}')
        # source_and_constrained_densities_cal_test = np.vstack([source_densities_cal_test, constrained_densities_cal_test_all_steps])
        
        # Tmin1_mixture_pdf_vals = mixture_pdf_from_densities_mat(source_and_constrained_densities_cal_test[:-1], mixture_weights)

        # proposal_pdf_values = simple_pdf_gpr(X_cal_test=Xaug_cal_test_split, var_pool_min_max_norm=var_pool_min_max_norms[-1], \
        #                                      exp_pool_sum_norm=exp_pool_sum_norms[-1], gpr_model=self.model, lmbda=lmbda)
        # proposal_pdf_values = source_and_constrained_densities_cal_test[-1]

        # unnormalized_mix_lik_ratios = proposal_pdf_values / Tmin1_mixture_pdf_vals ### 20250703

        # normalized_mixture_weights = unnormalized_mix_lik_ratios / np.sum(unnormalized_mix_lik_ratios)


        # print("normalized_mixture_weights[0:5] : ", normalized_mixture_weights[0:5])
        # print("normalized_mixture_weights[-5:] : ", normalized_mixture_weights[-5:])
