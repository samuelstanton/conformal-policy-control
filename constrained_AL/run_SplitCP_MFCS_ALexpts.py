...
#     parser.add_argument('--K_vals', nargs='+', help='Values of K to try', required = True)
    ...
    parser.add_argument('--noise_magnitude', type=float, default=0.05, help='Positive value controlling noise magnitude. 1 means noise is one std')
    ...
    if (dataset == 'airfoil'):
        ...
        errs_airfoil = compute_errors_for_measurements(X_airfoil, Y_airfoil) if heteroscedastic == 'Y' else None
        ...
        n_wine = len(Y_wine)
        errs_wine = compute_errors_for_measurements(X_wine, Y_wine) if heteroscedastic == 'Y' else None
        ...
        errs_communities = compute_errors_for_measurements(X_communities, Y_communities) if heteroscedastic == 'Y' else None
        ...
        Y_meps = meps_data[:,-1]
        n_meps = len(Y_meps)
        errs_meps = compute_errors_for_measurements(X_meps, Y_meps) if heteroscedastic == 'Y' else None
